...
# Configuración de Correo SMTP (cargada desde variables de entorno)
...
# Contraseña maestra para borrado de registros (cargada desde variable de entorno)
DELETE_MASTER_PASSWORD = os.environ.get('DELETE_MASTER_PASSWORD', '')
...
if app.secret_key == 'dev-only-change-in-production':
    import warnings
    warnings.warn('⚠️  SECRET_KEY no configurada — usando valor por defecto INSEGURO. Configura SECRET_KEY en .env', stacklevel=1)
...
def clear_history():
    ...
    if not DELETE_MASTER_PASSWORD or password != DELETE_MASTER_PASSWORD:
